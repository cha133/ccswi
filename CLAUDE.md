...
Each profile stores two models:
- **main** — maps to `ANTHROPIC_MODEL` + `ANTHROPIC_DEFAULT_OPUS_MODEL` (the main/default model)
- **fast** — maps to `ANTHROPIC_DEFAULT_SONNET_MODEL` + `ANTHROPIC_DEFAULT_HAIKU_MODEL` (the fast/small model)

Models with `_1m: true` get `[1m]` appended (e.g., `claude-opus-4-20250514[1m]`) to indicate 1-million token context support. Both primary and secondary support this flag.
