...
export function formatProfileTable(store: ProfilesStore): void {
  ...
  if (entries.length === 0) {
    info("No profiles yet. Run `ccswi add` to add one.");
    return;
  }
...
export function formatProfileDetail(profile: Profile, index?: number): void {
  ...
  console.log(`${pc.dim("main:")}      ${formatModelWith1m(profile.main, profile.main_1m)}`);
  console.log(`${pc.dim("fast:")}      ${formatModelWith1m(profile.fast, profile.fast_1m)}`);
}
